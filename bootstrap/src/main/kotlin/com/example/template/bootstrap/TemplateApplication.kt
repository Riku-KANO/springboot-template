...
import com.example.template.bootstrap.config.PaymentGatewayBeanRegistrar
import com.example.template.bootstrap.config.PersistenceBeanRegistrar
import com.example.template.bootstrap.config.UseCaseBeanRegistrar
 ...
 *
 * ##### 関数型 Bean 定義 (BeanRegistrarDsl) と @Configuration の使い分け
 ...
 * Spring 7 の `BeanRegistrarDsl` で明示的にコンストラクタ注入する
 * ([UseCaseBeanRegistrar], [PersistenceBeanRegistrar], [PaymentGatewayBeanRegistrar])。一方、Spring Security の
...
@Import(UseCaseBeanRegistrar::class, PersistenceBeanRegistrar::class, PaymentGatewayBeanRegistrar::class)
